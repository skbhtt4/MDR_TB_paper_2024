...
phy_tax <-  readRDS("../Processed_data/phy_mtph_hrze.rds")
...
write.csv(result_lme_dt,paste(tab_folder,"Tab_LME_ARG_HRZE.csv",sep = "/"))
...
names(coeff_dt_w)
coeff_dt_w$Visit2.Wk <- NA
coeff_dt_w$Visit1.Mo <-  NA
...
ht  =  Heatmap(log(mat_aro+ 1),name = "Log(RPKM + 1)",
               ...
               border = T, heatmap_legend_param = list(title = "Log(RPKM + 1)", 
                                                       ...
                                                       title_gp = gpar(fontsize = 14,fontface = "bold" ),
                                                       labels_gp = gpar(fontsize = 12,fontface = "bold" )))
pdf(paste0(results_folder,"/Fig_S4.pdf"),height = 3, width = 24,useDingbats = F)
draw(ht,heatmap_legend_side = c("top"),annotation_legend_side="right",legend_grouping = "original")
dev.off()
